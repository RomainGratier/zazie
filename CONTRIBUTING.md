...
Maintainers propose dependency upgrades manually. Automatic Dependabot version-update pull requests are disabled for npm and GitHub Actions. Automatic security-update pull requests are controlled separately in the repository settings and are also disabled. Dependency changes follow the same CI approval and review rules as other changes and are never automatically merged.
